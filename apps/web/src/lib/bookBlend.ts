...
import { defaultSettings, type BookRecord, type ReaderSettings } from '../types/book'
...
export function blendSettingsMatch(rec: BookRecord, ui: ReaderSettings): boolean {
  ...
  const snapSight =
    snap.sentenceTranslateAfterSightings ?? defaultSettings.sentenceTranslateAfterSightings
  const snapWhen =
    snap.sentenceTranslateWhen ?? defaultSettings.sentenceTranslateWhen
  const snapStyle =
    snap.sentenceTranslateStyle ?? defaultSettings.sentenceTranslateStyle
  return (
    snap.pairId === ui.pairId &&
    snap.paceGamma === ui.paceGamma &&
    snapCap === ui.learnWordCap &&
    snapSent === ui.sentenceTranslateEnabled &&
    snapSight === ui.sentenceTranslateAfterSightings &&
    snapWhen === ui.sentenceTranslateWhen &&
    snapStyle === ui.sentenceTranslateStyle
  )
}
